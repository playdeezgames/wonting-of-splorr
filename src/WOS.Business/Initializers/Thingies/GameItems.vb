Friend Module GameItems
    ...
    Friend Sub InitializeItems(data As WorldData)
        ...
        InitializeWeaponItem(data, SwordItemName, "Sword", "!"c, Hue.DarkGray, 6, 2, 50, equipSlot:=EquipSlot.Weapon)
        InitializeWeaponItem(data, ClubItemName, "Club", "$"c, Hue.Brown, 3, 1, 125, equipSlot:=EquipSlot.Weapon)
        InitializeWeaponItem(data, DaggerItemName, "Dagger", "%"c, Hue.DarkGray, 3, 1, 75, equipSlot:=EquipSlot.Weapon)
        InitializeWeaponItem(data, SpearItemName, "Spear", "&"c, Hue.Brown, 3, 2, 25, equipSlot:=EquipSlot.Weapon)
        InitializeWeaponItem(data, AxeItemName, "Axe", "'"c, Hue.DarkGray, 9, 3, 40, equipSlot:=EquipSlot.Weapon)

        InitializeArmorItem(data, WoodenShieldItemName, "Wooden Shield", "("c, Hue.Brown, 2, 1, 5, equipSlot:=EquipSlot.Shield)
        InitializeArmorItem(data, HelmetItemName, "Helmet", ")"c, Hue.DarkGray, 2, 0, 10, equipSlot:=EquipSlot.Head)
        InitializeArmorItem(data, LeatherArmorItemName, "Leather Armor", "*"c, Hue.Brown, 2, 1, 5, equipSlot:=EquipSlot.Torso)
        InitializeArmorItem(data, ShieldItemName, "Shield", "+"c, Hue.DarkGray, 2, 1, 25, equipSlot:=EquipSlot.Shield)
        InitializeArmorItem(data, ChainMailItemName, "Chain Mail", ","c, Hue.DarkGray, 2, 1, 10, equipSlot:=EquipSlot.Torso)
        InitializeArmorItem(data, PlateMailItemName, "Plate Mail", "-"c, Hue.Gray, 4, 2, 20, equipSlot:=EquipSlot.Torso)
    End Sub
